...
export function StatStop({ count }: { count: number }) {
  const stopClass = classNames(styles.stop, count > 0 && styles.blue);

  const svg = classNames(count > 0 && styles.svg);

  return (
    <div className={stopClass}>
      <div className={styles.descr}>
        <span className={styles.title}>Остановки</span>
        <span className={styles.stops}>{count}</span>
      </div>
